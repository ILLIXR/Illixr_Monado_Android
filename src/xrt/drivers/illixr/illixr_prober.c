                        ...
                        struct xrt_device **out_xdevs)
{
    ...
    illixr_comp = "libpose_prediction.so:libcommon_lock.so:libtimewarp_gl.so:librk4_integrator.so:libslam.so:libandroid_cam.so";//:libslam.so:librk4_integrator.so";//getenv("ILLIXR_COMP");
	if (!illixr_path ) {
