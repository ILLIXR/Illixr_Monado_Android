    ...
    namespace ILLIXR
{
	...
	struct cam_type : public switchboard::event
	{
		...
		cam_type(time_point _time, cv::Mat _img0, cv::Mat _img1) : time{_time}, img0{_img0}, img1{_img1} {}
	};
	...
	struct imu_integrator_input : public switchboard::event
	{
		...
		imu_integrator_input(time_point last_cam_integration_time_,
		                     duration t_offset_,
		                     imu_params params_,
		    ...
		    : last_cam_integration_time{last_cam_integration_time_}, t_offset{t_offset_}, params{params_},
		      biasAcc{biasAcc_}, biasGyro{biasGyro_}, position{position_}, velocity{velocity_}, quat{quat_}
		{}
	};
	...
	struct imu_raw_type : public switchboard::event
	{
		...
		imu_raw_type(Eigen::Matrix<double, 3, 1> w_hat_,
		    ...
		    : w_hat{w_hat_}, a_hat{a_hat_}, w_hat2{w_hat2_}, a_hat2{a_hat2_}, pos{pos_}, vel{vel_}, quat{quat_},
		      imu_time{imu_time_}
		{}
	};
	...
	struct pose_type : public switchboard::event
	{
		...
		pose_type()
		    : sensor_time{time_point{}}, position{Eigen::Vector3f{0, 0, 0}},
	...
	typedef struct vk_image_handle
	{
		...
		vk_image_handle(int fd_, int64_t format_, size_t alloc_size, uint32_t width_, uint32_t height_)
		    : file_descriptor{fd_}, format{format_}, allocation_size{alloc_size}, width{width_}, height{height_}
		{}
	} vk_image_handle;
	...
	typedef struct vk_buffer_handle
	{
		...
		vk_buffer_handle(AHardwareBuffer *ahardware_buffer_,
		                 int64_t format_,
		                 size_t alloc_size,
		                 uint32_t width_,
		                 uint32_t height_)
		    : ahardware_buffer{ahardware_buffer_}, format{format_}, allocation_size{alloc_size}, width{width_},
		      height{height_}
		{}
	} vk_buffer_handle;
	...
	struct image_handle : public switchboard::event
	{
		...
		image_handle() : type{graphics_api::TBD}, gl_handle{0}, num_images{0}, usage{swapchain_usage::NA} {}

		image_handle(GLuint gl_handle_, uint32_t num_images_, swapchain_usage usage_)
		    : type{graphics_api::OPENGL}, gl_handle{gl_handle_}, num_images{num_images_}, usage{usage_}
		{}

		image_handle(int vk_fd_,
		             int64_t format,
		             size_t alloc_size,
		             uint32_t width_,
		             uint32_t height_,
		    ...
		    : type{graphics_api::VULKAN}, vk_handle{vk_fd_, format, alloc_size, width_, height_},
		      num_images{num_images_}, usage{usage_}
		{}

		image_handle(AHardwareBuffer *ahardware_buffer,
		             int64_t format,
		             size_t alloc_size,
		             uint32_t width_,
		             uint32_t height_,
		    ...
		    : type{graphics_api::VULKAN},
		      vk_buffer_handle{ahardware_buffer, format, alloc_size, width_, height_}, num_images{num_images_},
		      usage{usage_}
		{}
	};

	// Used to identify which graphics API is being used (for swapchain construction)
	enum class semaphore_usage
	{
		LEFT_RENDER_COMPLETE,
		RIGHT_RENDER_COMPLETE,
		NA
	};
	...
	struct semaphore_handle : public switchboard::event
	{
		...
		semaphore_handle() : vk_handle{0}, usage{semaphore_usage::NA} {}

		semaphore_handle(int vk_handle_, semaphore_usage usage_) : vk_handle{vk_handle_}, usage{usage_} {}
	};

	struct illixr_signal : public switchboard::event
	{
		int illixr_ready;
		illixr_signal() : illixr_ready(false) {}

		illixr_signal(int illixr_ready_) : illixr_ready(illixr_ready_) {}
	};
	...
	struct rendered_frame : public switchboard::event
	{
		...
		rendered_frame(std::array<GLuint, 2> &&swapchain_indices_,
		               std::array<GLuint, 2> &&swap_indices_,
		               fast_pose_type render_pose_,
		               time_point sample_time_,
		               time_point render_time_)
		    : swapchain_indices{std::move(swapchain_indices_)}, swap_indices{std::move(swap_indices_)},
		      render_pose(render_pose_), sample_time(sample_time_), render_time(render_time_)
		{}
	};
	...
	struct texture_pose : public switchboard::event
	{
		...
		texture_pose(duration offload_duration_,
		             unsigned char *image_,
		             time_point pose_time_,
		             Eigen::Vector3f position_,
		             Eigen::Quaternionf latest_quaternion_,
		             Eigen::Quaternionf render_quaternion_)
		    : offload_duration{offload_duration_}, image{image_}, pose_time{pose_time_}, position{position_},
		      latest_quaternion{latest_quaternion_}, render_quaternion{render_quaternion_}
		{}
	};
} // namespace ILLIXR
